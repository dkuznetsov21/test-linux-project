...
export async function ensureReadyDestination(destinationDirectory, archivePath) {
...
export async function createZipArchive(outputsDirectory, output = process.stdout) {
  await runCommand('zip', ['-r', BUILT_SITES_ARCHIVE_NAME, BUILT_SITES_DIRECTORY_NAME], {
    cwd: outputsDirectory,
    stdio: ['ignore', 'ignore', 'inherit'],
  }).catch((error) => {
    throw new Error(`Failed to create ZIP archive: ${error.message}`);
  });

  output.write(`Archive created: ${path.join(outputsDirectory, BUILT_SITES_ARCHIVE_NAME)}\n`);
}
...
export class BuiltSiteArchiver {
  ...
  async run() {
    const sites = await findBuiltSites(this.outputsDirectory);

    if (sites.length === 0) {
      throw new Error(`No built site folders found in ${this.outputsDirectory}`);
    }
